...
def get_basic_info(cursor):
    queries = {
        ...
        "Total Sale Value (Last 3 Months)": """
            SELECT COALESCE(ROUND(SUM(ABS(se.change_quantity) * p.price), 2), 0) AS total_sale
        ...
        "Total Restock Value (Last 3 Months)": """
            SELECT COALESCE(ROUND(SUM(se.change_quantity * p.price), 2), 0) AS total_restock
            FROM stock_entries se
            JOIN products p ON se.product_id = p.product_id
        ...
        "Below Reorder & No Pending Reorders": """
            ...
            AND p.product_id NOT IN (SELECT DISTINCT product_id FROM reorders WHERE status = 'Pending')
        """
    }
    ...
    for label, query in queries.items():
        ...
        row = cursor.fetchone()
        # Use .get() or index 0 depending on cursor type.
        # Since we used dictionary=True in app.py:
